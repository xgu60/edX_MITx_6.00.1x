...
class SimpleVirus(object):
    ...
    def reproduce(self, popDensity):
        ...
        r_prob = self.maxBirthProb * (1 - popDensity)
        prob = random.random()
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    for k in range(300):
        popList[k] = popList[k] / float(numTrials)
    
        
    pylab.plot(timeList, popList)
        ...
        if prob < r_prob:
            for key in self.resistances.keys():
                ...
                if prob2 < self.mutProb:
                    self.resistances[key] = not self.resistances[key]
...
class TreatedPatient(Patient):
    ...
    def addPrescription(self, newDrug):
        ...
        if not (newDrug in self.adminDrugs):
    ...
    def update(self):
        ...
        for v in copy_viruses:
            if v.doesClear():
                ...
                try:
                    self.viruses.append(v.reproduce(popDensity, self.adminDrugs))
                except NoChildException:
                    continue
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    for i in range(numTrials):
        ...
        for j in range(150):
            popList1[j] += patient.update()
            popList2[j] += patient.getResistPop(['guttagonol'])
        ...
        for k in range(150, 300):
            popList1[k] += patient.update()
            popList2[k] += patient.getResistPop(['guttagonol'])
            
    for l in range(300):
        popList1[l] = popList1[l] / float(numTrials)
        popList2[l] = popList2[l] / float(numTrials)
        
    pylab.plot(timeList, popList1)
    pylab.plot(timeList, popList2)
    pylab.title('SimpleVirus simulation')
    pylab.xlabel('Time Steps')
    pylab.ylabel('Average Virus Population')
    pylab.legend()

    ...
    def singleLinkageDist(self, other):
        ...
        for p1 in self.points:
            for p2 in other.points:
                ...
                if tempDist == 0.0 or p1p2Dist < tempDist:
                    tempDist = p1p2Dist
        return tempDist
...
class ClusterSet(object):
    ...
    def mergeClusters(self, c1, c2):
        ...
        newCluster = Cluster(points, pointType)
    ...
    def findClosest(self, linkage):
        ...
        check = False
        tempDist = 0.0
        tupleList = [(self.members[0], self.members[0])]
        if len(self.members) > 1:
            for i in range(len(self.members)-1):
                for j in range(i+1, len(self.members)):
                    clusterDist = linkage(self.members[i], self.members[j])
                    if check == False or tempDist > clusterDist:
                        check = True
                        tempDist = clusterDist
                        tupleList.append((self.members[i], self.members[j]))
        return tupleList[-1]
    ...
    def mergeOne(self, linkage):
        ...
        tCluster = self.findClosest(linkage)
        self.mergeClusters(tCluster[0], tCluster[1])
        return tCluster
...
def test():
    points = buildCityPoints('C:/Users/Sheldon/Downloads/canopy/600 2x/ProblemSet6/cityTemps.txt', True)
